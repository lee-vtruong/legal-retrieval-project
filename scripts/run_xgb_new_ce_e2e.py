...
def run_end_to_end_new_ce():
    print("🚀 BƯỚC 1: Khởi động Hybrid Retriever & CROSS-ENCODER MỚI...")
    retriever = MultiViewHybridRetriever(load_models=True)
    
    # ĐIỂM SÁNG GIÁ NHẤT: Dùng mô hình vừa Fine-tune
    new_ce_path = str(MODELS_DIR / "fine_tuned_ce_alqac")
    print(f"🔥 Đang nạp vũ khí mới từ: {new_ce_path}")
    ce_model = CrossEncoder(new_ce_path, device="cuda:0")

    features_order = ["s_hybrid", "s_ce", "p_ce", "u_uncertainty", "s_gat"]
    TOP_K_RETRIEVAL = 30

    # ==========================================
    # PHẦN 1: HUẤN LUYỆN XGBOOST VỚI FEATURE MỚI
    # ==========================================
    print("\n🧠 BƯỚC 2: Trích xuất 5 Features trên tập TRAIN...")
    ...
    print("\n" + "="*50)
    print("🎯 KẾT QUẢ XGBOOST + FINE-TUNED CROSS-ENCODER")
